...
def fig_inter(py_code: str, fname: str) -> str:
    ...
    # 设置图像保存路径
    base_dir = r"/home/data-agent/public/"
    ...
    try:
        ...
        if fig:
            ...
            return f"![Data Visualization](/images/{image_filename})"
        else:
...
async def chat(query: QueryRequest):
    try:
        ...
        # 构建包含历史的消息列表
        messages = [msg for msg in session_histories[query.session_id]]
        ...
        # 更新会话历史
        session_histories[query.session_id] = messages + [("assistant", response)]
        
        # 限制历史消息数量，防止内存占用过大
        if len(session_histories[query.session_id]) > 20:
            session_histories[query.session_id] = session_histories[query.session_id][-20:]
